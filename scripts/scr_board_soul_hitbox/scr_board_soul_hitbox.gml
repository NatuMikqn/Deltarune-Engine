///SoulHitbox
function soul_hitbox()
{
	var _pos, _list, _data;
	with (obj_battle_soul){
		_pos = new Vector2(x, y)
		if (hitbox){
			_list = obj_battle_board.list;
			...
			for (var i = 0; i < array_length(_list); i++) {
				...
				if (_data.type == BOARD_TYPE.POLYGON){
					...
					if ((_heart_count % 2) == 0){
						...
						target_hitbox.hit = true;
						target_hitbox.dir = _dir;
						target_hitbox.ccw = true;
						//event_user(0);
					}else{
						target_hitbox.hit = false;
					}
				}
